...
class PageAdminUploadTests(TestCase):
    ...
    def test_mixin_builds_complete_upload_panel(self):
        ...
        self.assertEqual(fieldsets[1][1]["fields"], (
            "image_file", "media_asset_preview", "image",
            "image_title", "image_alt_text", "image_description",
        ))

    def test_mixin_prefills_existing_asset_metadata(self):
        asset = MediaAsset.objects.create(
            title="Existing title", alt_text="Existing alt", description="Existing description",
            file=SimpleUploadedFile("existing.txt", b"content", content_type="text/plain"),
        )
        ...
        self.assertEqual(form.fields["image_description"].initial, "Existing description")
    ...
    def test_upload_attaches_new_asset_and_keeps_old_one(self):
        ...
        page = Page.objects.create(title="Replaceable", slug="replaceable", image=old_asset)
        form = self.admin.get_form(self.request, page)(
            {"title": page.title, "slug": page.slug, "body": "", "published": "", "image": old_asset.pk,
             "image_title": "New title", "image_alt_text": "New alt", "image_description": "New description"},
            {"image_file": SimpleUploadedFile("new.bin", b"new", content_type="application/octet-stream")},
            instance=page,
        )
